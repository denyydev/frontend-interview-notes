...
## 47. Как обрабатываются события в React
...
function Button() {
  ...
  return (
    <button onClick={() => handleClick(1, 'Алекс')}>
      Нажми меня
    </button>
  );
}
...
function Button() {
  ...
  return (
    <button onClick={handleClick.bind(null, 1, 'Алекс')}>
      Нажми меня
    </button>
  );
}
...
## 49. Как работает onChange в input
...
## 50. Как сделать controlled input
...
## 51. Поднятие состояния (lifting state up)
...
## 52. Однонаправленный поток данных
...
function App() {
  ...
  return (
    <Parent count={count} onIncrement={() => setCount(count + 1)} />
  );
}
...
function Header() {
  return <Navigation />;  {/* Не передаём user */}
}

function Navigation() {
  return <UserMenu />;  {/* Не передаём user */}
}
...
function App() {
  ...
  return (
    <Header theme={user.theme}>  {/* Передаём только theme */}
      <Navigation>
...
## 54. Когда стоит выносить логику в хук
